from tkinter import*          # part:-1, time:- complete / part:- 2, time:- 01:40 , time:-07:40 something check
import math,random,os          # part-3, time:- 09:00 and after pending
...
class Bill_App:                
    def __init__(self,root):
        ...
        F2.place(x=5,y=195,width=425,height=450)
        ...
        F3.place(x=440,y=195,width=425,height=450)
        ...
        F4.place(x=875,y=195,width=425,height=450)
        ...
        F5.place(x=1310,y=195,width=615,height=450)
        ...
        F6.place(x=0,y=650,relwidth=1,height=170)
        ...
        btn_F.place(x=890,width=980,height=110)
        ...
        GBill_btn=Button(btn_F,text="Genrate Bill",command=self.bill_area,bg="cadetblue",fg="white",bd=2,pady=15,width=12,font="arial 15 bold").grid(row=0,column=1,padx=5,pady=5)
        Clear_btn=Button(btn_F,text="Clear",command=self.clear_data,bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 15 bold").grid(row=0,column=2,padx=5,pady=5)
        Exit_btn=Button(btn_F,text="Exit",command=self.Exit_app,bg="cadetblue",fg="white",bd=2,pady=15,width=12,font="arial 15 bold").grid(row=0,column=3,padx=5,pady=5)
        Share_btn=Button(btn_F,text="Share",bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 15 bold").grid(row=0,column=4,padx=5,pady=5)
        self.welcome_bill() 
  

  #==============coding Dev start, no.:-A1 ===========================
       #========Bill About============================
        F7=LabelFrame(self.root,bd=10,relief=GROOVE,text="Bill Information",font=("times new roman",15,"bold"),fg="gold",bg=bg_color)
        F7.place(x=0,y=825,relwidth=1,height=170)

        d1_lbl=Label(F7,text="Information",bg=bg_color,fg="white",font=("times new roman",14,"bold")).grid(row=2,column=2,padx=20,pady=1,sticky="w")
        #d1_txt=Entry(F7,width=80,textvariable=self.information,font="arial 10 bold",bd=7,relief="SUNKEN").grid(row=2,column=3,padx=10,pady=1)
        d1_txt=Entry(F7,width=20,font="arial 10 bold",bd=7).grid(row=2,column=3,padx=10,pady=1)
 



  #==============coding Dev End, no.:-A1 ==============================
    ...
    def save_bill(self):  
        ...
        if op>0:
            ...
            f1=open("billing_app/bills/"+str(self.bill_no.get())+".txt","w")
    ...
    def find_bill(self):
        present="no"  
        for i in os.listdir("billing_app/bills/"):
              if i.split('.')[0]==self.search_bill.get():
                    f1=open(f"billing_app/bills/{i}","r")
                    self.txtarea.delete('1.0',END)
                    for d in f1:
